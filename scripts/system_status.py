...
output_result = ""
output_result += "  " + "\n"
output_result += "========== Initialize the x120x... ==========" + "\n"

f = open("/tmp/system_status", "w")
...
def read_cpu_temp():
    try:
        output = check_output(["cat", "/sys/class/thermal/thermal_zone0/temp"]).decode("utf-8")
        metric_str = output.strip()
        return float(metric_str) / 1000
    except (CalledProcessError, ValueError) as e:
        print(f"Error reading cpu hardware metric: {e}")
        return None
...
def display_status(shutdown):
    ...
    elif pld_state != 1 and capacity <= 10 and not shutdown:
        shutdown = True
        warn_status = "UPS Power failure imminent! Auto shutdown in 1-2 minutes!"
        shutdown_func()
    elif pld_state != 1 and shutdown:
        warn_status = "UPS Power failure imminent! Auto shutdown to occur within 5 minutes!"
    elif pld_state == 1 and shutdown:
        warn_status = "[TEST] AC Power has been restored. Auto shutdown has been cancelled!"
        shutdown = False
    else:
        warn_status = ""
